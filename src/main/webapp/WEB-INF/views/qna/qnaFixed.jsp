...
<body>
	<div class="contentContainer">
		...
		<form name="detailForm" id="detailForm">
			<input type="hidden" name="qna_Num" id="qna_Num"> <input
				...
				<table summary="검색">
					...
					<tr>
						<td id="btd1"><label>검색조건</label> <select id="search"
							...
							<input type="button" value="문의 게시판" id="qnamooni"
		...
		<div id="qnaList">
			<table summary="게시판 리스트">
				...
				<thead>
					<tr>
						<th data-value="qna_Num" class="order">글번호 <c:choose>
								<c:when
									test="${data.order_by=='qna_Num' and  data.order_sc=='ASC'}">▲</c:when>
								<c:when
									test="${data.order_by=='qna_Num' and  data.order_sc=='DESC'}">▼</c:when>
								<c:otherwise>▲</c:otherwise>
							</c:choose>
						</th>
						<th>글제목</th>
						<th data-value="qna_Date" class="order">작성일 <c:choose>
								<c:when
									test="${data.order_by=='qna_Date' and  data.order_sc=='ASC'}">▲</c:when>
								<c:when
									test="${data.order_by=='qna_Date' and  data.order_sc=='DESC'}">▼</c:when>
								<c:otherwise>▲</c:otherwise>
							</c:choose>
						</th>
					...
					<c:choose>
						<c:when test="${not empty qnaList}">
							<c:forEach var="qna" items="${qnaList}" varStatus="status">
								<tr class="tac" data-num="${qna.qna_Num}">
									<td>${count - status.index}</td>
									<td class="goDetail tal">${qna.qna_Title}</td>
									<td>${qna.qna_Date}</td>
									<td class="name">${qna.qna_Name}</td>
								</tr>
							</c:forEach>
						</c:when>
						<c:otherwise>
